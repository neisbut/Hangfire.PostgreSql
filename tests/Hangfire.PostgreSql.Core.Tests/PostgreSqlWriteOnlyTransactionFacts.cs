...
values ('', '', now() at time zone 'utc') returning ""id""";
			...
			UseConnection(sql =>
			{
				...
				Commit(sql, x =>
				{
					x.GetType().GetMethod("ExpireJob").Invoke(x, new object[] { jobId, TimeSpan.FromDays(1) });
				});
...
values ('', '', now() at time zone 'utc', now() at time zone 'utc') returning ""id""";
			...
			UseConnection(sql =>
			{
				...
				Commit(sql, x =>
				{
					x.GetType().GetMethod("PersistJob").Invoke(x, new object[] { jobId });
				});
...
values ('', '', now() at time zone 'utc') returning ""id""";
			...
			UseConnection(sql =>
			{
				...
				Commit(sql, x =>
				{
					x.GetType().GetMethod("SetJobState").Invoke(x, new object[] { jobId, state.Object });
				});
...
returning ""id""";
			...
			UseConnection(sql =>
			{
				...
				Commit(sql, (x) =>
				{
					x.GetType().GetMethod("AddJobState").Invoke(x, new object[] { jobId, state.Object });
				});
